...
gs = gridspec.GridSpec(nrows=3, ncols=len(d_list), wspace=0.3,hspace=0.6)
mue_set = []   
...
#--- do bootstrapping ----
    if len(times) > 0: 
        ...
        ax0.plot([min(times), max(times)],[len(times)/2.0,len(times)/2.0], color='red', alpha = 0.5, linestyle='dashed',)
        index_x_half = np.argwhere(s[0] > np.max(s[0])/2)[0][0]
        tau = ret_time(times)    
        if index_x_half > 0: x_half = (s[1][index_x_half]+s[1][index_x_half-1]/2) 
        else:  x_half = s[1][index_x_half]
       # ax0.plot([x_half,x_half],[0,len(times)/2.0], color='red', alpha = 0.5)
...
plt.title("Residence times for "+str(t)+" replicas, mean: "+str(np.round(np.mean(mue_set),2))+"  std: "+str(np.round(np.std(mue_set),2)),fontsize=12)
